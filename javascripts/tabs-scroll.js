function initTabScroll() {
  ...
  if (!tabs || !list) return false;
  if (tabs.querySelector(".md-tabs-scroll-btn")) return true;
  ...
  function updateIndicators() {
    ...
    if (atStart) { tabs.classList.remove("scrolled-right"); }
    else { tabs.classList.add("scrolled-right"); }

    if (atEnd || !isScrollable) { tabs.classList.add("scrolled-end"); }
    else { tabs.classList.remove("scrolled-end"); }
...
// Try immediately, retry a few times if DOM not ready yet
if (!initTabScroll()) {
  var attempts = 0;
  var timer = setInterval(function () {
    if (initTabScroll() || ++attempts > 20) clearInterval(timer);
  }, 100);
}
